...
Phase 5 輸出 `session_notes`（一句話 handoff）。`investment_protocol_v5_0` Phase 0 三層 cache 的 L1 會優先讀取 `sector_intel.json` 的 macro_regime / exposure_ceiling / hot_sectors / binary_risks，**跳過 web search**，節省 ~10k token。
