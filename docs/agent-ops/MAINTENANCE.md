...
| 7 | 測試全部**直呼被測函式**，繞過真實呼叫點（2026-08-09） | 把 check 從 `main()` 拆掉、閘完全不生效，測試照樣全綠。「函式行為對」不等於「閘生效」；同 V4.116.2「`invest` 從沒進 `PROTOCOL_VALIDATORS`」。**判準：把這個 check 從入口刪掉，測試會紅嗎？** 每道閘至少一個案例走完整入口 |
| 8 | producer 與 consumer 各寫一份路徑／欄位名（2026-08-09） | 一致時全綠；任一方改動後 consumer 讀不到 → 走「找不到就靜默」分支 → **rc=0 永久靜音**，不報錯。共用字串一律 import。當 consumer 對「缺席」的處理是靜默時，重複定義的代價從報錯升級成靜音 |
