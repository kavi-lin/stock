...
> **狀態（2026-08-08，V4.112.0）**：使用者已逐項拍板。
> **B2 / B3 / B7 / B1 四項已按下表建議執行**（第一波，CHANGELOG `[4.112.0]`）——
> B7 的語意題定調為 `< 60`（非極端即可），並一併做了 `downside_deviation` 的 NaN 修復。
> **B4 / B5 / B6 維持現狀**，重啟前置條件見 `TODO.md`。
> 本檔以下內容保留為決策紀錄，**不是待辦清單**。
