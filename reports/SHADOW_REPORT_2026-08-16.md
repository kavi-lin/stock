# Shadow Report — 2026-08-16

> 唯讀讀出端。history entries scanned: 192。

## #2 Anchor Dispersion（backfill — 立即可用）

- 樣本 n=80，CV min/P33/median/P66/max = 0.033 / 0.392 / 0.4568 / 0.5406 / 1.3224
- 現行門檻 high<0.15 / low>0.35（拍腦袋初值）
- **建議門檻（33/66 pct）**: high<0.392 / low>0.5406
- 建議門檻 = 歷史 CV 33/66 percentile（三等分 high/medium/low）。#2 cap 接線前由 user 核可。

## #6 Owner-Earnings 倍數 shadow

- 累積 18/20 session；翻轉 8 筆（rate=0.4444）；decision: **accumulating**
- checkpoint: ≥20 session 且翻轉率 <15% → 切換提案

## #3 Valuation Archetype shadow

- 累積 45/20 session；翻轉率 0.3333；archetype 分布 {'hypergrowth': 21, 'balanced': 18, 'cyclical': 3, 'financial': 3}
...
- 來源 `*_pf_quant.json`（同標的取最新一次）；全母體 n=6，其中 anchor live **n=1**
- **live cohort**（判準來源）隱含 PE min/P33/median/P66/max = 241.3 / 241.3 / 241.3 / 241.3 / 241.3
- 全母體（背景脈絡，混 archetype，**不是**校準目標）= 13.5 / 21.7 / 26.8 / 31.9 / 241.3；帶內 3/6，高於上限 2，低於下限 1
- 現行 clamp [15.0, 35.0]；上限實際綁到 2/6（rate=0.3333）
- 折現率：beta fallback 0/6（rate=0.0，資料品質訊號——偏高就該重量 `FWD_BETA_FALLBACK`）；折現率 clamp 綁到 2/6
- **判準**: accumulating（live cohort 1/5）
...
| NVDA | ineligible | 2030-01 | 29.2 | 15.0 | -19.8 | lower | 114.9 | 223.96 |
| PLTR | ineligible | 2029-12 | 37.4 | 35.0 | 97.07 | upper | 160.97 | 172.01 |
| RKLB | eligible | 2029-12 | 241.3 | 35.0 | 84.24 | upper | 12.01 | 82.83 |

## Lane Model 分層哨兵（V4.0.0 — Sent/News/Tech → Sonnet 監測）

- 近 10 session（baseline n=80）lane mean drift：
  - fundamentals: baseline 1.35 → recent 2.0（drift 0.65）
  - sentiment: baseline 0.39 → recent 0.81（drift 0.42）
  - news: baseline 1.68 → recent 1.74（drift 0.06）
  - technical: baseline 0.74 → recent 1.3（drift 0.56）
- 近 10 val DISAGREE: 0；polarization 異常: 4
