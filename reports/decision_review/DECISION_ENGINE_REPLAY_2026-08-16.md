# Decision Engine Replay — 2026-08-16
...
| 分類 | 筆數 |
|---|---|
| history trades 總數 | 184 |
| replayed（current rules） | 11 |
...
- match: **6/11**
- forward-validation rule drift: **2**（1.0.x 歷史資料，預期差異）
- unexplained mismatch: **3** → 需逐筆 triage
...
| 2026-08-07 | AAOI | `calculation_steps` | +0.0763 | +0.0763 | +0.0000 | HOLD | HOLD | ✓ match |
| 2026-08-09 | BAC | `calculation_steps` | +1.0558 | +1.0558 | +0.0000 | STAGED_ENTRY | STAGED_ENTRY | ✓ match |
| 2026-08-09 | BAC | `calculation_steps` | +1.1112 | +1.1112 | +0.0000 | STAGED_ENTRY | STAGED_ENTRY | ✓ match |
| 2026-08-09 | BAC | `calculation_steps` | +0.7789 | +0.8000 | +0.0211 | HOLD | STAGED_ENTRY | ✗ NEEDS_TRIAGE |
| 2026-08-09 | NOW | `calculation_steps` | +1.0855 | +0.9770 | -0.1085 | STAGED_ENTRY | STAGED_ENTRY | ✗ NEEDS_TRIAGE |
| 2026-08-09 | NOW | `calculation_steps` | +1.0616 | +1.2155 | +0.1539 | STAGED_ENTRY | STAGED_ENTRY | ✗ forward_validation_rule_drift |
| 2026-08-09 | PLTR | `calculation_steps` | +0.5165 | +0.6063 | +0.0898 | HOLD | HOLD | ✗ forward_validation_rule_drift |
| 2026-08-10 | META | `calculation_steps` | +0.1271 | +0.1305 | +0.0034 | HOLD | HOLD | ✓ match |
| 2026-08-10 | RKLB | `calculation_steps` | +0.5310 | +0.5935 | +0.0625 | HOLD | HOLD | ✗ NEEDS_TRIAGE |
| 2026-08-16 | SNDK | `calculation_steps` | +1.1195 | +1.1195 | +0.0000 | STAGED_ENTRY | STAGED_ENTRY | ✓ match |

**當期規則但因未記錄輸入而排除的筆數（advisory 對照，非 parity 證據）**

這 4 筆決策日 ≥ 2026-07-16、lane 輸入齊全，只因 `structural_shift.tier` 等欄位未持久化、且 sweep 顯示會改變結果而排除。取 spec 預設值重算的結果列於下——一致代表 engine 與當時 PM 手算同號，但因假設成立與否無法證實，不計入 §2 的 parity 分母。
...
| 2026-08-10 | NVDA | `calculation_steps` | +1.0166 → +1.0166 | +0.0000 | HOLD → STAGED_ENTRY | ✗ |
...
> session_export_version='V4.6' predates the 5-lane weighting (replayable: V5.0, V5.1, V5.2, V5.3)
...
### `decision_sensitive_unknown` — 39 筆
...
| `gates.binary_event_within_48h` | 9 |
| `red_team.counter_evidence_strength` | 5 |
| `transition.rule2_eligible` | 3 |

- advisory（全部未知取 spec 預設：tier=NONE / strength=4 / rule2=off / binary_event=off）與存檔一致：**3/39**
...
| 2026-08-10 | NVDA | +1.0166 → +1.0166 | +0.0000 | HOLD → STAGED_ENTRY | ✗ |
...
### `macro_inputs_unavailable` — 1 筆

> phase0_macro_snapshot missing macro_multiplier/backdrop_score

| date | ticker | version | detail |
|---|---|---|---|
| 2026-08-07 | NET | V5.3 | phase0_macro_snapshot missing macro_multiplier/backdrop_score |
