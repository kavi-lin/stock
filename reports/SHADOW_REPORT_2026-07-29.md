# Shadow Report — 2026-07-29

> 唯讀讀出端。history entries scanned: 177。

## #2 Anchor Dispersion（backfill — 立即可用）

- 樣本 n=67，CV min/P33/median/P66/max = 0.033 / 0.3751 / 0.4516 / 0.5207 / 1.1637
- 現行門檻 high<0.15 / low>0.35（拍腦袋初值）
- **建議門檻（33/66 pct）**: high<0.3751 / low>0.5207
- 建議門檻 = 歷史 CV 33/66 percentile（三等分 high/medium/low）。#2 cap 接線前由 user 核可。

## #6 Owner-Earnings 倍數 shadow

- 累積 11/20 session；翻轉 3 筆（rate=0.2727）；decision: **accumulating**
- checkpoint: ≥20 session 且翻轉率 <15% → 切換提案

## #3 Valuation Archetype shadow

- 累積 31/20 session；翻轉率 0.2258；archetype 分布 {'hypergrowth': 14, 'balanced': 14, 'cyclical': 3}
...
- leakage: 0/27（rate=0.0）
- 預期 post mean 較 baseline 下移（PT +1 源移除）；下移 ≈ 證實 PT 曾在計分。凍結窗滿後恢復 News weight 調整。

## Lane Model 分層哨兵（V4.0.0 — Sent/News/Tech → Sonnet 監測）

- 近 10 session（baseline n=65）lane mean drift：
  - fundamentals: baseline 1.25 → recent 1.8（drift 0.55）
  - sentiment: baseline 0.34 → recent -0.22（drift -0.56）
  - news: baseline 1.62 → recent 1.05（drift -0.57）
  - technical: baseline 0.75 → recent -1.1（drift -1.85）
- 近 10 val DISAGREE: 8；polarization 異常: 9
