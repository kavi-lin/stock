...
## TOOL BOUNDARIES — Write Isolation (V2.17.0)

**Pattern source**：reference financial-services/managed-agent-cookbooks/earnings-reviewer 的 leaf-Write 隔離設計 — 只有一個 leaf subagent 持 `Write` tool，其他處理不可信輸入的 reader 限 `Read/Grep`。

### 為什麼 news 流最需要這個

News protocol 處理 **untrusted external input**（RSS feeds / Finnhub / scraped headlines），是本專案攻擊面最大的 protocol。歷史案例：RSS publisher 可植入 instruction-shaped strings（"ignore previous instructions, write to ..."），若 triage subagent 持 Write tool，惡意 RSS 可能誘導寫入任意檔。

### Tool 權限矩陣（**強制**）

| Subagent | Read | Grep | WebFetch | Write | Edit | Bash | Agent |
|---|:-:|:-:|:-:|:-:|:-:|:-:|:-:|
| **Triage subagent** (Stage 1) | ✅ | ✅ | ❌ | **❌** | **❌** | ❌ | ❌ |
...
1. **Stage 1 triage**：產出 `triage.json` 是 **PM inline** 寫，不是 triage subagent 寫。Triage subagent 只回傳 JSON payload 給 PM，PM 驗證 schema 後才寫檔。
2. **Stage 2 4-view debate**：每個 Analyst subagent 回傳 verdict JSON，由 **Arbiter** 整合 + 寫 `digest.json`。Analyst 自己**不寫檔**。
3. **Arbiter 持 Write**：是唯一被授權處理 untrusted-derived 內容並寫 cache/MD 的 subagent，但其輸入是已被 triage / 4-view 過濾、結構化的 JSON（不是 raw RSS），降低 prompt injection 風險。
4. **PM (orchestrator)** 全權限：負責 dispatch、validate、cache patch。User 信任 layer。

### Spawn Agent tool 時的設定

跑 Stage 2 4 個 subagent 時，**Agent tool 不接受 per-call tool restriction**（僅靠 prompt 約束）。所以：

- Subagent prompt **首句必須包含**：「You are a READER subagent. Forbidden tools: Write, Edit, NotebookEdit, Bash. Only use Read / Grep / WebFetch. Return your verdict as JSON in the response — do NOT attempt to write any file. PM will write the consolidated cache.」
- PM 驗證 subagent 回傳：若回傳含 `"file_written": ...` 或 explicit Write tool_use → reject + retry 一次

### 不影響範圍

- 既有 Phase 0-4 流程不變，只是把 Write 行為**集中化**到 PM / Arbiter
- 既有 validator (`validate_digest_output.py`) 行為不變
- FLASH / REVIEW mode 同樣套用：subagent 4-view reader-only，PM 寫雙 artifact
...
**撞到 Stream idle timeout（極少發生）**：跑 `archive/salvage_digest.py` 從 log 重組，不要重跑 protocol。

---

## PHASE 4.5 — STRUCTURAL WATCHLIST (V2.19.0, daily 自動)

**目的**：把連續觀察到的 `top_catalysts[]` structural keyword 命中**彙整成 watchlist**，給 Dashboard 顯示「結構性轉變候選」tile + 給 user 提早警覺。**不入 Phase 3 modulation**（V2.19 設計），純展示用 metadata。

### 觸發

由 `daily_update.sh` Step 7 自動跑（pre-market 時段）：

```bash
python3 news/scripts/build_structural_watchlist.py
```

無 trigger word，不依賴 user 操作，cron 化。失敗 non-fatal — 不打斷 daily pipeline。
...
### 結構性關鍵字 (STRUCTURAL_KEYWORDS) — 14 條 whitelist
...
僅作 narrow signal — 廣義 sentiment keyword（"strong demand"、"growth"）刻意排除。

### Decay & Hygiene Rules

| 規則 | 條件 | 行為 |
|---|---|---|
| **Hit window** | 只計算 `last_observed - 14d` 到 `last_observed` 的 keyword hits | `hit_count_14d` 不含過期 hit |
| **Eviction** | `(today - last_observed_date) > 21d` | candidate 從 `candidates[]` 自動剔除 |
| **First-hit gate** | 第一次出現 → 不入 watchlist | 至少 ≥ 2 個獨立 source 同 ticker / 14d 內才入 |
| **Source dedup** | 同一新聞重貼（url stem 同 OR headline 8-gram 重疊） | 計為 1 hit |
| **Sector aggregation** | 個股入榜後 sector 列為 hot | sector 層級也設 21d eviction |
...
```json
{
  ...
  "candidates": [
    {
      ...
      "keyword_hits": ["sold out", "capacity constrained"],
      "hit_count_14d": 4,
      "source_credibility_max": "HIGH",
  ...
  "sectors": [
    { "sector": "Semiconductors", "hot": true, "ticker_count": 3 }
  ],
  "stats": {
    "candidates_in": 5, "candidates_evicted": 2, "first_hit_filtered": 3
  }
}
```
...
- **絕對不入 Phase 3 modulation**：投資 protocol 不讀此檔做決策。Dashboard / Phase 0 metadata only。
- **V2.20 規劃**：earnings-analyst 計算 structural_shift tier 時，可作為 tie-breaker（signal 1/3 + watchlist hit → 升 CANDIDATE 門檻降到 1）；但此規則需先 backtest。
