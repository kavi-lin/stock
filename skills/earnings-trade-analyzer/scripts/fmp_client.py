...
class FMPClient:
    ...
    def get_earnings_calendar(self, from_date: str, to_date: str) -> Optional[list]:
        ...
        # v3 /earning_calendar deprecated; stable uses /earnings-calendar
    ...
    def get_company_profiles(self, symbols: list[str]) -> dict[str, dict]:
        ...
        for i in range(0, len(symbols), batch_size):
            ...
            # v3 batched profile deprecated; stable takes single symbol per call
            # We loop here per ticker (small batches → still rate-limited globally)
            data = []
            for sym in batch:
                url = f"{self.STABLE_URL}/profile"
                resp = self._rate_limited_get(url, {"symbol": sym})
                if resp:
                    data.extend(resp if isinstance(resp, list) else [resp])
            if data:
                self.cache[cache_key] = data
                for profile in data:
                    if isinstance(profile, dict):
    ...
    def get_historical_prices(self, symbol: str, days: int = 250) -> Optional[list[dict]]:
        ...
        # v3 /historical-price-full deprecated; stable uses /historical-price-eod/full
        url = f"{self.STABLE_URL}/historical-price-eod/full"
        params = {"symbol": symbol, "limit": days}
        data = self._rate_limited_get(url, params)
        if data:
            # Stable returns either a list directly OR {"historical": [...]} depending on plan
            result = data["historical"] if isinstance(data, dict) and "historical" in data else data
            if result:
