...
Lives in skills/_shared/ (moved from momentum-monitor/scripts in the V4.5
audit cleanup) — single source for OHLCV fetch + RSI across skills.
Consumers: momentum-monitor/momentum.py + technical-analyst/analyze.py (via
shim at the old path), theme-detector/etf_scanner.py, market-sentiment-
analyzer/sentiment.py, thematic-screener/screen.py, short-term-target/
predict.py.
...
def _fetch_fmp_ohlc(ticker, period):
    ...
    try:
        # Routed through the central pool (shared 250/min budget). Returns None
        # on any failure → caller falls back to yfinance.
        import sys
