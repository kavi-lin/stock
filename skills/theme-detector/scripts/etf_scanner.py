...
class ETFScanner:
    ...
    FMP_HIST_BATCH_SIZE = 1   # stable/historical-price-eod/full: single symbol only (batch = 402)
    FMP_QUOTE_BATCH_SIZE = 1  # stable/quote: single symbol only under current plan (batch = 402)
    ...
    def _parse_historical_response(self, data: Any, result: dict) -> None:
        """Parse FMP historical response.

        Handles two formats:
        - Legacy dict: {"symbol": "X", "historical": [{date, close...}]}
          or {"historicalStockList": [...]}
        - New stable flat list: [{symbol, date, open, high, low, close, volume}, ...]

        Keys in result are always normalized (e.g., BRK.B not BRK-B).
        """
        if isinstance(data, list):
            # New stable/historical-price-eod/full format: flat list grouped by symbol
            grouped: dict[str, list[dict]] = {}
            for row in data:
                sym = self._normalize_symbol_for_fmp(row.get("symbol", ""))
                if sym:
    ...
    def _get_etf_volume_ratio_yfinance(self, symbol: str) -> dict:
        ...
        try:
            ...
            volume = data["Volume"].squeeze().dropna()  # squeeze handles MultiIndex columns from yfinance
            if len(volume) < 20:
                return result
    ...
    def _batch_stock_metrics_yfinance(self, symbols: list[str]) -> list[dict]:
        ...
        for symbol in symbols:
            ...
            try:
                if len(symbols) == 1:
                    sym_data = data
                    # Newer yfinance returns MultiIndex (Ticker, Field) columns for single ticker
