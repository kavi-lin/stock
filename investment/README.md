...
> 當前檔案：`investment_protocol_v4_6.md`
...
總體掃描     記憶載入    五大分析師    衝突仲裁       決策引擎              雙軌執行規劃     Session 存檔
(可快取)                (+Burry)     (含 T4)     raw → bonus → macro    (Aggressive+Conservative)
```
...
Step 2: 若 4 agent 同向且 Burry 不 veto → × 1.15 (consensus bonus)
...
| Agent | 職責 | 預設權重 | Skill |
|---|---|---|---|
| Fundamentals Analyst | P/E, FCF, 營收成長 | 0.30 | `us-stock-analysis` |
| Sentiment Analyst | Reddit/X, Put/Call, VIX | 0.20 | `market-sentiment-analyzer` |
| News Analyst | 48h 新聞、分析師評級 | 0.20 | `market-news-analyst` |
| Technical Analyst | MA, RSI, MACD, 量能 | 0.30 | `technical-analyst` |
| **Contrarian (Burry)** | **估值錨 + veto check** | **獨立，不加權** | **`short-contrarian-analyst`** |
| Risk Manager | 倉位 + 尾部風險 | — | `portfolio-risk-manager`, `tail-risk-analyzer` |
...
- `burry_score = 2`（高估）→ `DOWNGRADE_DECISION`（BUY → HOLD）或 `OVERRIDE_BURRY`（需說明動能/基本面如何超越估值疑慮）
...
| Consensus Bonus | 4 agent 同向 + Burry 不 veto | raw × 1.15 |
