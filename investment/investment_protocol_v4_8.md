...
3. **若 cache 新鮮 → PM 抽 thin 摘要進 EARNINGS_ANALYST_BUNDLE**:
   ...
   EARNINGS_ANALYST_BUNDLE = {
     ...
     "valuation": {
       ...
       "dcf_levered_intrinsic":  (ea.get("valuation") or {}).get("dcf_levered_intrinsic"),  # V1.74
       ...
       "pt_dispersion_pct":      (ea.get("valuation") or {}).get("pt_dispersion_pct"),      # V1.74
       "pt_news":                (ea.get("valuation") or {}).get("pt_news") or [],          # V1.74 最近 8 筆機構 PT 調整
     },
     "analyst": {                                                                            # V1.74
       "rating_trend":   (ea.get("analyst") or {}).get("rating_trend"),    # improving|stable|declining
       "grades_summary": (ea.get("analyst") or {}).get("grades_summary"),  # strongBuy%, total_analysts
       "grades_news":    (ea.get("analyst") or {}).get("grades_news") or [], # 近 10 筆升降評事件
     },
...
### Phase 1 資料層（V4.10 新增）— Peer Comparison Bundle

Phase 1 末段 PM **MUST** 取得同業 peer set，供 Fundamentals + Burry lane 做 relative valuation 量化（取代「貴／便宜」定性論述）。成本：2-7 次 FMP call（含 24h cache，同 ticker 後續 session 0 call）。

1. **取 peer 與 profile**（一律走 `skills/_shared/company_context` cache，**禁止**重打 `/stable/profile`）:
   ```python
   import sys
   ...
   peers_raw = get_peers(TICKER)[:5]   # 取 top-5；FMP 通常回 5-10 個
   peer_profiles = {p: get_profile(p) for p in peers_raw}
   peer_profiles = {p: prof for p, prof in peer_profiles.items() if prof}  # drop None
   ```

2. **計算 PEER_BUNDLE**:
   ```python
   def _med(field):
       vals = [prof.get(field) for prof in peer_profiles.values()
               if prof.get(field) is not None]
...
3. **失敗 / 樣本不足判定**:
   - `len(peer_profiles) < 3` → `PEER_BUNDLE = {"status": "insufficient_peers", "peers": list(peer_profiles.keys())}`
   - `get_peers()` 回 `[]`（FMP 無 peer 資料 / network error）→ `PEER_BUNDLE = {"status": "unavailable"}`
   - 上述兩種狀態下 Fundamentals / Burry lane 自動 fallback 到 sector median rubric（既有規則），**不**中止 protocol

4. **注入規則**:
   - **僅** Fundamentals + Burry lane prompt 含 `PEER_BUNDLE`
   - Sentiment / News / Technical lane prompt **不得**包含此 bundle（避免 cross-lane anchoring）
   - PM 在 Phase 2.5 conflict resolution 時亦不得用 PEER_BUNDLE 數字作裁量依據（peer median 只是 lane 內部證據）

5. **物理隔離契約**:
   - PEER_BUNDLE 純讀，不得改
   - 24h cache 命中時 0 FMP call；cache miss 1 ticker × ~6 call（1 peers + 5 profiles）

---

### Phase 1 資料層（V4.11 新增）— FMP Supplementary Bundle

Phase 1 末段 PM **MUST** 取得 FMP 補充束，供 Fundamentals / Sentiment / News / Burry 4 個 lane 取得 dual-fetch (Finnhub) 與 EARNINGS_ANALYST_BUNDLE 之外的高價值 FMP 欄位。成本：2-6 次 FMP call（含 24h cache，同 ticker 同日後續 session 0 call）。
...
1. **取得 bundle**：
   ...
   # 失敗（API key 缺 + 無 cache）→ FMP_SUPP_BUNDLE = None
   ```

2. **Bundle schema（V1.0）**：
   - `quality_scores`: Altman Z-Score / Piotroski F-Score（含 `altman_zone` ∈ {danger, grey, safe} 與 `piotroski_strength` ∈ {weak, moderate, strong}）
   - `owner_earnings`: Buffett owner earnings（latest quarter + qoq_growth）
   - `insider_summary`: 季度內部人交易統計（v1.79.0 起填）
   - `institutional`: 機構持股 QoQ 變化（v1.80.0 起填）
   - `congressional_trades`: senate + house 議員交易（v1.84.0 起填）
   - `ma_events`: 近 90 日 M&A 事件（v1.84.0 起填）

3. **失敗判定**：
   - `FMP_SUPP_BUNDLE is None` → 整段 prompt 注入省略；4 lane 標註「FMP_SUPP_BUNDLE: unavailable — fallback to skill-internal sources」
   - 個別 section 為 `{}` → 該 section 對應 rubric 自動 skip（lane 不得猜值）

4. **注入規則**：
   - ✅ Fundamentals lane：`quality_scores` + `owner_earnings`
   - ✅ Sentiment lane：`insider_summary` + `institutional` + `congressional_trades`
   - ✅ News lane：`ma_events`（補充 sec_filings_recent）
   - ✅ Burry lane：`quality_scores` + `owner_earnings` + `insider_summary`
   - ❌ Technical lane：不傳（OHLCV-only lane）
   - PM 在 Phase 2.5 conflict resolution **不得**直接引用 supp bundle 欄位作裁量依據（lane 內部證據）

5. **物理隔離契約**：
   - FMP-only。**不得**混入 dual_fetch `scoring.*` 欄位，亦不得從 supp bundle 推回 Finnhub 欄位
   - 24h cache @ `skills/_shared/fmp_supp_cache/<TICKER>_<DATE>_supp.json`
   - bundle 欄位純讀，禁止 lane subagent 改寫
...
- **EARNINGS_ANALYST_BUNDLE 使用規則**(V1.71)— 若 prompt 含此 bundle:
  ...
  - **V1.74 新增欄位使用規則**：
    - `valuation.pt_dispersion_pct` > 40% → reasoning 加注「分析師目標價分歧高（{val}%），共識參考度下降」（不自動扣分）
    - `analyst.rating_trend == "improving"` → Fundamentals score +0.5（分析師共識轉強，替換舊 +0.5 邏輯，依據更確定）
    - `analyst.rating_trend == "declining"` → Fundamentals score -0.5
    - `analyst.grades_summary.strong_buy_pct` > 50% → 在 reasoning 注記「機構評等偏多（強買佔 {val}%，共 {total} 分析師）」
    - `analyst.grades_news` 近 10 筆：升評數 > 降評數 → +0.5 信心加成；降評數 ≥ 3 且 > 升評數 → -0.5
    - `valuation.pt_news` 最近 3 筆平均 PT 調整：整體上調 → 論點引用；整體下調 → score -0.5
...
- **PEER_BUNDLE 使用規則**(V4.10 新增)— 若 prompt 含 PEER_BUNDLE:
  - **量化 relative valuation**：將 dual-fetch `peRatio` 與 `PEER_BUNDLE.peer_pe_median` 比較。差異 > 30% 觸發強訊號（高估 → score -1，低估 → score +1）；差異 > 50% → ±2
  - **論述格式**：避免「P/E 偏高」這種定性說法。改寫為「P/E 35× vs 同業中位數 22× = +59% premium，鎖定下行修正風險」
  - `peer_market_cap_median` 用於 size cohort 校驗：若 ticker market cap < peer median × 0.3 → 規模顯著偏小，rubric 可加 size discount note（**不**自動扣分）
  - `peer_beta_median` 用於風險定位：若 ticker beta > peer median + 0.3 → 在 reasoning 註記「波動較同業高」
  - **絕對禁止**：(a) 把 peer median 當 lane score；(b) cross-reference Sentiment / News / Technical lane（cross-lane anchoring 違規）；(c) 用 PEER_BUNDLE 數字推測其他 lane 的結論
  - 若 `PEER_BUNDLE.status` 為 `insufficient_peers` 或 `unavailable` → 整段省略，fallback 到既有 sector median rubric（V4.9 規則第 2 條 P/E vs sector median）
- **FMP_SUPP_BUNDLE 使用規則（V4.11 新增）**— 若 prompt 含 `FMP_SUPP_BUNDLE`：
  - **品質分數**（`quality_scores.altmanZScore` + `piotroskiScore`）：
    - `altman_zone == "danger"` (Z < 1.81) → Fundamentals score -1（補強既有 D/E rubric）
    - `piotroski_strength == "strong"` (F ≥ 7) → Fundamentals score +1（盈餘品質 9-point check 通過）
    - `piotroski_strength == "weak"` (F ≤ 2) → Fundamentals score -1
  - **Owner Earnings**（`owner_earnings.ownersEarnings` + `qoq_growth`）：
    - `qoq_growth > 0.15` (環比 +15%) → reasoning 註記「owner earnings 環比擴張」（不直接 +score，避免雙計 — FCF 已在主 rubric）
    - `qoq_growth < -0.30` → score -0.5（現金生產力衰退警訊）
  - **絕對禁止**：把 `quality_scores.altmanZScore` 數字當 lane score；不得 cross-ref Sentiment/Burry lane 是否同 trigger
  - 若 `FMP_SUPP_BUNDLE` 為 None / `quality_scores` 為 `{}` → 整段 skip，本段規則不適用
...
- **RUBRIC**:
  ...
  - **FMP_SUPP_BUNDLE.institutional 規則（V4.11 新增）**：若 prompt 含 `FMP_SUPP_BUNDLE.institutional.accumulation_signal`：
    - `accumulation_signal == "accumulating"`（QoQ +1% 以上）→ stock_specific +1
    - `accumulation_signal == "distributing"`（QoQ -1% 以下）→ stock_specific -1
    - `put_call_ratio_change_qoq > 0.2` → stock_specific -0.5（避險增加）
    - `accumulation_signal == "neutral"` 或欄位為 `{}` → 不調整
  - **FMP_SUPP_BUNDLE.congressional_trades 規則（V4.11 新增；v1.86.0 起 bundle 才有此欄位）**：若 `congressional_trades.net_signal`：
    - `net_signal == "bullish"` → stock_specific +0.5（國會議員淨買；STOCK Act 有資訊不對稱意涵）
    - `net_signal == "bearish"` → stock_specific -0.5
...
- **8-K filings 規則（V4.11 新增）**：fetch.py 新欄位 `sec_8k_filings[]` 為 30 日內 SEC 8-K 重大事件（M&A、CEO 異動、訴訟、重組、年報延遲）。News subagent 必須瀏覽該 list；若有 8-K 但無對應 headline → narrative 加註「8-K filed YYYY-MM-DD（事件解讀依 finalLink）」並 score +/-0.5（取決方向，例如 leadership change 可正可負由 LLM 判定）
- **FMP_SUPP_BUNDLE.ma_events 規則（V4.11 新增；v1.86.0 起 bundle 才有此欄位）**：若 `ma_events.events[]` 非空 → ticker 為 acquirer/target，News score 觸發強訊號（target 通常 +1 / acquirer 視 transaction_value 而定）。需在 `key_factors` 中提及。
...
**PEER_BUNDLE 額外校驗（V4.10 新增）**：Burry inline 在 burry_score 計算後，可讀 PEER_BUNDLE 做 mispricing 對照。若 `EV/EBIT > peer_pe_median × 1.5`（或 fcf_yield < peer 中位數一半）→ 在 `burry_voice` 文字加註「relative-to-peers expensive: ...」narrative。**禁止**因 PEER_BUNDLE 數字直接調整 burry_score 或 verdict（保 deterministic）。若 PEER_BUNDLE.status 為 `insufficient_peers` / `unavailable` → 整段省略。

**FMP_SUPP_BUNDLE Burry 規則（V4.11 新增）**：Burry inline 必須讀 `FMP_SUPP_BUNDLE.quality_scores` + `FMP_SUPP_BUNDLE.owner_earnings` + `FMP_SUPP_BUNDLE.insider_summary`，按下列確定性規則調整 `burry_score`，並在 `burry_voice` 標註觸發理由：

1. **Altman Z-Score gate**（`quality_scores.altman_zone`）：
   - `altman_zone == "danger"` (Z < 1.81) → `burry_score -= 2`，`burry_voice` 加註「財務困境風險 Altman Z={z:.2f}」
   - `altman_zone == "grey"` (1.81 ≤ Z < 2.99) → `burry_score -= 1`，`burry_voice` 加註「財務灰色帶 Altman Z={z:.2f}」
   - `altman_zone == "safe"` (Z ≥ 2.99) → 不調整
2. **Piotroski F-Score filter**（`quality_scores.piotroski_strength`）：
   - `piotroski_strength == "strong"` (F ≥ 7) → `burry_score += 1`（品質溢價）
   - `piotroski_strength == "weak"` (F ≤ 2) → 不調整 score，但 `burry_voice` 加註「low quality F={f}」（**不**強制 verdict，保留 LLM 收尾權）
3. **Owner Earnings vs GAAP FCF 差異**（`owner_earnings.ownersEarnings` vs `cash_flow[0].freeCashFlow`）：
   - 若兩者皆有值且差異絕對值 > 30% → `burry_voice` 加註「SBC/maintenance capex 調整顯著：owner_earnings={oe} vs GAAP_FCF={fcf}」（純 narrative，不調 score）
4. **Insider summary trend**（`insider_summary.latest_trend`）：
   - `latest_trend == "accumulating"` 且既有 `insider_pts == 0` → 將 `insider_pts` 升至 1（Burry 認可內部人 buying）
   - `latest_trend == "distributing"` → `burry_voice` 加註「insider distributing 4Q (acq/dis ratio={r:.2f})」（不直接 -score，避免雙計）

5. **Levered vs Unlevered DCF 差異（V1.74 新增）**（`EARNINGS_ANALYST_BUNDLE.valuation.dcf_levered_intrinsic` vs `dcf_intrinsic`）：
   - 兩者皆有值且差距絕對值 > 20% → `burry_voice` 加註「capital structure gap: FCFF={dcf_intrinsic:.0f} vs FCFE={dcf_levered:.0f}，差距 {gap:.0%}，槓桿對估值影響顯著」（純 narrative，不調 score）
   - 差距方向：levered < unlevered → 高槓桿侵蝕股權價值；levered > unlevered → 低成本債務放大 equity return
   - 若 `EARNINGS_ANALYST_BUNDLE` 不含 `dcf_levered_intrinsic` / 值為 null → 整段 skip

任何單一規則的 score 調整以 ±2 為上限。所有調整必須在 `burry_voice` 留可追溯字串。若 `FMP_SUPP_BUNDLE` 為 None / 對應 section 為 `{}` → 該規則整條 skip，不阻斷流程。
...
Agent(
  ...
  HARD CONSTRAINTS:
    ...
    - 若資料有缺漏，填「N/A」而非自行補全。
    - **Score Scale 強制規範（V1.88 統一）**：
      * Final Score：寫成 `X.XXX / 3.0` 或裸數字（值 ≤ 3.0）。**禁止** `/5.0`、`/10` 標註。
      * 4 lane 個別 score（Fundamentals / Sentiment / News / Technical）：寫成 0-3 範圍裸數字（強訊號 +3 / 反向 -3）。**禁止** `lane: X/10` 或 `lane: X/5.0` 形式。
      * Burry score：寫成 `X.X / 100`。**禁止** `/12`（早期 bug）或 `/10`。
      * Red Team strength：寫成 `N/5` 或 `N` 整數。
      * 範例正確：`| **Final Score** | 1.609 / 3.0 |`、`Burry: 50.3 / 100`、`Fundamentals BUY 3.0`
      * 範例錯誤：`HOLD (6.72/10 risk-adjusted)`、`Fundamentals: 8.1/10`、`Burry 4.5/12`
...
### Phase 5 Step 5 — Markdown Score-Scale Validation Gate（V1.88 新增）

Sonnet 寫完 MD 後，PM **MUST** 跑：
...
驗證行為：
- **rc=0**：通過 → 結束 protocol
- **rc=1**：score scale drift（如 `/5.0`、`/10`、Burry `/12`）→ 必須處理：
  1. **Retry Sonnet 一次**：把 stderr 違規清單 paste 進 prompt 末尾，要求 Sonnet 修正並重寫 MD（同檔覆蓋）
  2. **Retry 仍 rc=1** → PM **inline 覆寫** MD（**禁止**用 freestyle，必須照 reports/20260502_TEAM.md template）
  3. PM fallback 寫完後 **再跑一次 validator**，rc=0 才結束 protocol

**禁止**：跳過 validator、直接接受 rc=1 報告、把 freestyle MD 寫進 reports/ 然後 protocol 標 done。
