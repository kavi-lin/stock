# Multi-Agent Investment Protocol (V4.3)
...
3. **Prior Session**: 讀取 `./invest_logs/history.json`，取最近一筆作為 prior context。
4. **Output Format**: 邏輯輸出為 JSON；Markdown 僅用於最終 Visualization Table。
5. **key_factors**: 最多 3 條，每條最多 8 個英文單字。
6. **Phase 5**: session export 只存結果，不重複 Phase 3 的計算步驟。
