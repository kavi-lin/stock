...
- [ ] **L4b-2 — 翻預設（skip 生效）**（依賴 L4b shadow 樣本，**使用者拍板**）：gate 接進 `shadow_report.py` 讀出端；停止規則建議用「discovery 覆蓋率穩定後再數 N」而非固定 session 數（暖機期命中率不反映穩態）；validator 目前擋 `shadow_only=false`，翻預設時要一併鬆綁。
