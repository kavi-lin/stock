...
- [ ] **風控三支 Batch B 餘三項——備忘結論為「維持現狀」,重啟需先解決前置條件**(證據見 `docs/plan_risk_trio_B.md`):
  - **B4 遷 technical_core**:~~前置為除息調整~~ **前置已解除(2026-08-08 下午)**。`/stable/historical-price-eod/dividend-adjusted` 在現行方案可用且回 `adjClose`;用它重跑同批 16 檔 shadow **翻轉 1 → 0**、bar 數 250/254 → 250/250,配息股 dd 漂移 +1.14 → -0.000、TLT 7.74→10.72 變 7.74→7.74。**閘門通過。** 但遷移的正確順序變成「先修共用模組再遷三支」,見下一項。
- [ ] **`technical_core` 主源與 fallback 的調整慣例不一致**(2026-08-08 探 B4 前置時發現,**獨立提案,需拍板**):`_fetch_fmp_ohlc` 取未調整 `close`,yfinance fallback 走 `auto_adjust=True`(已除息調整)——**同一檔標的拿到哪一種序列取決於當下 FMP 有沒有失敗**,而 fallback 是常態性靜默發生。改端點為 `historical-price-eod/dividend-adjusted`(欄名 `adjClose`)可讓模組內部自洽,並順帶消掉 `technical_core.py` 註解自陳的「配息股 MA 讀數略高」偏差。**Blast radius:9 個消費者**(technical-analyst / momentum-monitor / quant-backtest ×2 / kill_trigger_monitor / short-term-target predict / thematic-screener / market-sentiment-analyzer / theme-detector etf_scanner),RSI/MA/MACD/backtest/momentum 分數全會微幅移動。**動手前需各消費者關鍵輸出的前後對照 shadow**,尤其 quant-backtest(策略績效)與 short-term-target(其 `weights.yaml` 是使用者手動校準禁區)。此項完成才輪到 B4 遷三支。
