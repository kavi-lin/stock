...
# Non-fatal: wrap in `set +e` so a network blip on FRED doesn't kill the
# whole pipeline. Without this wrap the `if [ $? -eq 0 ]` was dead code —
# `set -e` would have aborted on the python3 line before the check ran.
FRED_STATUS="skipped"
...
if [ -f "$ETF_META" ]; then
  ...
  if [ -n "$LAST_REFRESH" ]; then
    ...
    if [ $DAYS_OLD -ge 90 ]; then
      ...
      # `set -o pipefail` so REFRESH_RC reflects python's rc, not `tail`'s
      # (which is always 0). Without it, a refresh failure was invisible.
      set +e
      set -o pipefail
