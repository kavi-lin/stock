...
### 層 D — FTD Detector cache（量化底部確認）
4. 檢查 `./ftd_cache/ftd_detector_YYYY-MM-DD_*.json`（取最新）
   - **存在** → 直接讀取
   - **不存在** → 執行腳本（約 10 秒）：
     ```bash
...
### 層 E — Market Top Detector cache（量化頂部偵測）
5. 檢查 `./market_top_cache/market_top_YYYY-MM-DD_*.json`（取最新）
   - **存在** → 直接讀取
   - **不存在** → 執行腳本（約 10 秒）：
     ```bash
...
```json
{
  ...
  "exposure_ceiling": "40-60% (from breadth composite.exposure_guidance)",
